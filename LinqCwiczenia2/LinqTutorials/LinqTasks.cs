...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => e.Job == "Backend programmer").Where(e => e.Salary > 1000);
        ...
        {
            IEnumerable<Emp> result = Emps.Where(em => em.Salary == Emps.Max(e => e.Salary));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (x, y) => x);
            return result;
        }
        ...
        {
            Emp result = Emps.OrderByDescending(e => e.HireDate)
                                .First();
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            var tmp = new { Ename = "Brak wartości", Job = string.Empty, HireDate = (DateTime?)null };
            var l = new List<object>
            {
                tmp
            };
            result = Emps.Select(x => new { x.Ename, x.Job, x.HireDate }).Union(l).ToList();
            return result;
        ...
        {
            IEnumerable<object> result = Depts.Select(d => new
            {
                name = d.Dname,
                numOfEmployees = Emps.Count(e => e.Deptno == d.Deptno)
            }).Where(d => d.numOfEmployees >= 1);
            return result;
        }
        ...
        {
            int result = arr.GroupBy(info => info)
                        .Select(group => new
                        {
                            Key = group.Key,
                            Count = group.Count()
                        }).Where(s => s.Count % 2 != 0).Select(s => s.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Select(d => d).Where(d => Emps.Count(e => e.Deptno == d.Deptno) == 5 || !Emps.Any(e => e.Deptno == d.Deptno)).OrderBy(d=>d);
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            return emps
                .Join(emps, e => e, em => em.Mgr,(e, em) => e)
                .Distinct()
                .OrderBy(e => e.Ename)
